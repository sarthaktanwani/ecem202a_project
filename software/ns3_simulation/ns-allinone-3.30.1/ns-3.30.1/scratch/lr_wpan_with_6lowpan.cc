...
//  LogComponentEnable ("LrWpanNetDevice", LOG_LEVEL_FUNCTION);
  LogComponentEnable ("SixLowPanNetDevice", LOG_LEVEL_INFO);//LOG_LEVEL_ALL);
...
//#endif
  ...
  uint32_t packetSize = 100;//10;
  uint32_t maxPacketCount = 100;//5;
  Time interPacketInterval =  MilliSeconds(1.); //Seconds (1.);
  Ping6Helper ping6;
