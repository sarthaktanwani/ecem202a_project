...
// void
// Monitor (std::string context, Ptr<const Packet> packet, uint16_t channel, WifiTxVector tx,
...
//   {
//     std::cout << "Addr1: " << hdr.GetAddr1() << "\tAddr2: " << hdr.GetAddr2() << std::endl;
//     std::cout << "Seq No. 1: " << hdr.GetSequenceNumber() << (hdr.IsData() ? ", DATA" : ", NO DATA") << std::endl;
//   }
// }
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("dataLen", "Length of data to be sent", dataLen);

  cmd.Parse (argc, argv);

  std::cout << "\ndata Length that will be sent would be: "<< dataLen << std::endl;
  ...
  pointToPoint.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (6560)));
...
//  UdpEchoClientHelper echoClient (csmaInterfaces.GetAddress (nCsma), 9);
  ...
  // echoClient.SetAttribute ("PacketSize", UintegerValue (dataLen));

  UdpEchoClientHelper Client (wifiInterfaces.GetAddress(0), 9);
  Client.SetAttribute ("MaxPackets", UintegerValue (1));
  Client.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  Client.SetAttribute ("PacketSize", UintegerValue (dataLen));
