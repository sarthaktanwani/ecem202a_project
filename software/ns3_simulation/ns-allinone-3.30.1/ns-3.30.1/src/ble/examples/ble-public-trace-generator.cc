...
NS_LOG_COMPONENT_DEFINE ("BleBroadcast");
  ...
  double length = 49; //<! Square room with length as distance 30
...
int main (int argc, char** argv)
{
  ...
  for (uint8_t iterationI=0;iterationI<nbIterations;iterationI++){
    ...
    for (uint32_t nodeI = 0; nodeI < nNodes; nodeI++)
    {
      ...
      {
        DynamicCast<BleNetDevice>(
            bleNetDevices.Get(nodeI))->GetPrivacyManager()->SetRandInterval(rand_invl);//rand_invl
      }
      DynamicCast<BleNetDevice>(
    ...
    for (uint32_t sniffI = nNodes; sniffI < nNodes+nSniffers; sniffI++)
    {
      ...
      {
        DynamicCast<BleNetDevice>(
            sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetRandInterval(rand_invl);//rand_invl
      }
      DynamicCast<BleNetDevice>(
