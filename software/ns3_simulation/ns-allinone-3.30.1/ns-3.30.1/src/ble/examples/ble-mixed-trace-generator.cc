...
NS_LOG_COMPONENT_DEFINE ("BleMixed");
  ...
  int nbIterations = 1;//25
  double length = 14; //<! Square room with length as distance 49
  ...
  bool single_device = false; //single or multiple device classes
  ...
  uint32_t nNodes = 25; // Number of nodes 25
  uint32_t nSniffers = 1; // Number of sniffers 1
  ...
  char *dev_names[4] = {"Frequent Emitter", "Moderate  Emitters", "Semi-Moderate  Emitter", "Low  Emitters"};
...
Transmitted (Ptr<OutputStreamWrapper> stream, const Ptr<const Packet> packet)
{
    ...
    uint8_t buffer_dst[6];
    header.GetDestAddr().CopyTo(buffer_dst);
    uint32_t addr_dest = buffer_dst[5];

    *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << " Packet transmitted  " << packet << " src addr = " 
                              << addr << "\t" << "dest addr:" << addr_dest << std::endl;

    //NS_LOG (LOG_INFO, "Packet transmitted  " << packet << " src addr = " << addr);
...
// save that a message has been received
	void
Received (Ptr<OutputStreamWrapper> stream, const Ptr<const Packet> packet)
{
  ...
  void
ReceivedError (Ptr<OutputStreamWrapper> stream, const Ptr<const Packet> packet)
{
...
// save that a message has been uniquely received
  void
ReceivedUnique (Ptr<OutputStreamWrapper> stream, const Ptr<const Packet> packet,
        const Ptr<const BleNetDevice>  netdevice, const uint32_t node_id)
{
  ...
  copy->RemoveHeader(header);
    ...
    *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << " Rx  " << "\t" << " src " 
	...
	void
ReceivedBroadcast (Ptr<OutputStreamWrapper> stream, const Ptr<const Packet> packet, 
    const Ptr<const BleNetDevice>  netdevice, const uint32_t node_id)
{
	Ptr<Packet> copy = packet->Copy();
	BleMacHeader header;
  ...
  void
TXWindowSkipped (Ptr<OutputStreamWrapper> stream, const Ptr<const BleNetDevice> nd)
{
  //NS_LOG (LOG_DEBUG, "Packet received  " << packet);
	uint8_t buffer[6];
...
void
MAC_GT (Ptr<OutputStreamWrapper> stream, const uint32_t id_node, const std::string this_mac)
{
...
int main (int argc, char** argv)
{
  ...
  cmd.AddValue ("num_sniffers", "Number of sniffers", nSniffers);
  ...
  NS_LOG_INFO ("BLE Mixed traffic file");
  ...
  for (uint8_t iterationI=0;iterationI<nbIterations;iterationI++){
    ...
    NS_LOG (LOG_INFO, "Ble Mixed traffic setup starts now");

    NodeContainer bleDeviceNodes;
    bleDeviceNodes.Create(nNodes);

    NodeContainer sniffers;
    sniffers.Create(nSniffers);
    ...
    Ptr<OutputStreamWrapper> sniff_stream = ascii.CreateFileStream ("./traces/"+std::to_string(iterationI)+"_"+"ble_sniffer_"+std::to_string(nNodes)+"_"+std::to_string(mobility_profile)+"_"+std::to_string(device_class)+".tr"); 
    Ptr<OutputStreamWrapper> gt_stream = ascii.CreateFileStream ("./traces/"+std::to_string(iterationI)+"_"+"mac_gt_"+std::to_string(nNodes)+"_"+std::to_string(mobility_profile)+"_"+std::to_string(device_class)+".tr");
    ...
    else if ( dynamic && mobility_profile==3 )
      mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel", "Mode", StringValue ("Time"),
                              "Time", StringValue ("2s"),
                              "Speed", StringValue ("ns3::ConstantRandomVariable[Constant=1.5]"),
    ...
    // Locate Sniffers
    MobilityHelper location_sniffer;
    Ptr<ListPositionAllocator> snifferPositionList = 
      CreateObject<ListPositionAllocator> ();
    for (uint32_t snifferPositionsAssigned = 0; 
        snifferPositionsAssigned < nSniffers; snifferPositionsAssigned++)
    {
      double x,y;
      x = length/2;//randT->GetInteger(0,length);
      y = length/2;//randT->GetInteger(0,length);
      *gt_stream->GetStream () << "SnifferId:" << "\t" << snifferPositionsAssigned << "\t"  << x << "\t" << y << std::endl;
      snifferPositionList->Add (Vector (x,y,1.0));
    }
    location_sniffer.SetPositionAllocator (snifferPositionList);
    location_sniffer.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
    location_sniffer.Install(sniffers);
    ...
    bleNetDevices = helper.Install (bleDeviceNodes, true);//nodes are configured to only transmit




    // Create the sniffers
    NetDeviceContainer sniffNetDevices;
    sniffNetDevices = helper.Install (sniffers, false);//sniffer is configured to receive too, obviously
    ...
    for (uint32_t nodeI = 0; nodeI < nNodes; nodeI++)
    {
      ...
      {
        DynamicCast<BleNetDevice>(
            bleNetDevices.Get(nodeI))->GetPrivacyManager()->SetRandInterval(xval->GetValue(180, 900));//rand_invl
      }
      DynamicCast<BleNetDevice>(
      ...
      {
        DynamicCast<BleNetDevice>(
          bleNetDevices.Get(nodeI))->GetPrivacyManager()->SetDeviceClass(random_integer);
        dev_class.push_back(random_integer);
      }
    ...
    // Set sniffer addresses
    NS_LOG (LOG_INFO, "Set sniffer addresses");
    for (uint32_t sniffI = nNodes; sniffI < nNodes+nSniffers; sniffI++)
    {
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetStream(gt_stream);
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetMacMap(mac_map);
      if (rand_invl>0)
      {
        DynamicCast<BleNetDevice>(
            sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetRandInterval(xval->GetValue(180, 900));//rand_invl
      }
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->UpdateAddress(sniffI);
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetDeviceClass(0);

      IRK_idendev_pairs.emplace_back( DynamicCast<BleNetDevice> (sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->GetIRK(), 
                        DynamicCast<BleNetDevice>(sniffNetDevices.Get(sniffI-nNodes))->GetIdentityAddress48() );

      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->SetBleStandard(0);
      NS_LOG (LOG_INFO, "Sniffer address = " 
          << DynamicCast<BleNetDevice>(sniffNetDevices.Get(sniffI-nNodes))->GetAddress ());
    }
    ...
    for(unsigned int j=0; j< IRK_idendev_pairs.size(); j++)
    {
      ...
      {
        DynamicCast<BleNetDevice>(
            sniffNetDevices.Get(sniffI-nNodes))->GetPrivacyManager()->AddIRK(IRK_idendev_pairs[j].first, IRK_idendev_pairs[j].second);
      }
   ...
   //helper.CreateAllLinks (bleNetDevices, scheduled, nbConnInterval);
    ...
    allDevices.Add(bleNetDevices);
    allDevices.Add(sniffNetDevices);
    helper.CreateBroadcastLink (allDevices, 
    ...
    ApplicationContainer apps = helper.GenerateBroadcastTraffic (randT, 
        bleDeviceNodes, pktsize, 0, packetSendDuration, interval, mobility_profile, dev_class);
    ...
    NS_LOG (LOG_INFO, " Generate data teaffic");
    ...
    for (uint32_t i=0; i< sniffNetDevices.GetN(); i++)
    {
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(i))->TraceConnectWithoutContext (
          "MacTx",MakeBoundCallback(&Transmitted, sniff_stream));
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(i))->TraceConnectWithoutContext (
          "MacRxBroadcast",MakeBoundCallback(&ReceivedBroadcast, sniff_stream));
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(i))->TraceConnectWithoutContext (
          "MacPromiscRx",MakeBoundCallback(&Received, sniff_stream));
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(i))->TraceConnectWithoutContext (
          "MacRxError",MakeBoundCallback(&ReceivedError, sniff_stream));
      DynamicCast<BleNetDevice>(
          sniffNetDevices.Get(i))->TraceConnectWithoutContext (
          "TXWindowSkipped",MakeBoundCallback(&TXWindowSkipped, sniff_stream));
    }

    // Data traffic trace callbacks

    for (uint32_t i=0; i< bleNetDevices.GetN(); i++)
    {
      DynamicCast<BleNetDevice>(
          bleNetDevices.Get(i))->TraceConnectWithoutContext (
          "MacRx",MakeBoundCallback(&ReceivedUnique, sniff_stream));
    }
