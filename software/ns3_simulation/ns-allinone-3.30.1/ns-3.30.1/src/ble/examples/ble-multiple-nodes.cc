...
NS_LOG_COMPONENT_DEFINE ("BleMultipleNodesExample");
  ...
  int interval = 4; //!< Time between two packets from the same node 
...
Transmitted (const Ptr<const Packet> packet)
{
    ...
    //NS_LOG (LOG_DEBUG, "Packet transmitted  " 
    ...
    const Ptr<const BleNetDevice>  netdevice)
{
	...
	copy->RemoveHeader(header);
	uint8_t buffer[2];
    netdevice->GetAddress48().CopyTo(buffer);
    uint32_t addr = buffer[1];
    //NS_LOG (LOG_DEBUG, "Packet unique received  " 
    //<< packet << " dest addr = " << addr);
...
int main (int argc, char** argv)
{
  ...
  NS_LOG_INFO ("BLE Multiple nodes example file");
 ...
 // helper.EnableAsciiAll (ascii.CreateFileStream ("example-ble.tr"));
  AsciiTraceHelper ascii;
  m_stream = ascii.CreateFileStream ("example-multiple-nodes.csv");
